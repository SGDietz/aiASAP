...
const BLOCKED_PROMPT_RE =
  /\b(?:contact|contacts|named g|for g|with g|call g|text g|email g|remind|reminder|notify|notification|g's|change subject|confirm understanding|review key points|check understanding|develop\s+\w+\s+strateg|learn\s+\w+\s+tip|practice\s+\w+\s+technique|to\s+do\s+list|grocery\s+list|walmart\s+list|shopping\s+list|gift\s+list|honey[-\s]?do\s+list|christmas\s+list|birthday\s+list|meaningful|(?:that|our)\s+bond|common\s+ground|set\s+better|build\s+stronger|(?:find|build)\s+deeper|build\s+trust\s+daily|trust\s+daily|create\s+your\s+vision|next\s+move|money\s+moves|smart\s+investments|\bmvp\b|\bkpi\b|\bmrr\b|\barr\b|\broi\b|minimum\s+viable|build\s+your\s+faith|set\s+faith\s+goals|grow\s+your\s+belief|spiritual\s+(?:habits|goals)|faith\s+goals|build\s+faith|build\s+your\s+focus|clarify\s+your\s+\w+|find\s+your\s+priorities|find\s+priorities|focus\s+your\s+\w+|align\s+your\s+\w+|optimize\s+your\s+\w+|sharpen\s+your\s+\w+|streamline\s+your\s+\w+|find\s+(?:\w+\s+)?income|find\s+(?:\w+\s+)?hustle|find\s+(?:\w+\s+)?streams|explore\s+(?:\w+\s+)?ideas|explore\s+(?:passive|active|new)\s+\w+|find\s+(?:passive|active|new)\s+\w+|discover\s+(?:\w+\s+)?income|make\s+your\s+move|just\s+begin\s+now|begin\s+now|start\s+small\s+today|start\s+small\s+now|ask\s+for\s+examples?|ai[-\s]?asap\s+style|build\s+real\s+connection|plan\s+(?:a|your)\s+meetup|set\s+a\s+date|talk\s+it\s+out|find\s+common\s+interests?)\b/i;
