...
OUT = ROOT / "public" / "social-artwork" / "v12"
...
ASSET_ITEMS = [
    ("Real mobile font proof", "aiasap-real-mobile-font-proof-v12.png", "review", "Real app CSS logo proof"),
...
def skew_layer(layer: Image.Image, degrees: float = -18.0, scale_x: float = 0.94) -> Image.Image:
...
def make_font_proof() -> Path:
    ...
    d.text((90, 790), "Arial Black / stronger mobile-style italic lean / scaleX(0.94) / brown-gold gradient", font=font(FONT_BOLD, 30), fill=(210, 158, 96))
    return save(img, "aiasap-real-mobile-font-proof-v12.png")
...
def make_youtube_banner() -> Path:
    ...
    # YouTube shows the full 2560x1440 on TVs, a 2560x423 desktop strip,
    # and only the 1546x423 center safe zone on phones. Keep the brand
    # signals inside the center zone while letting the full TV canvas read
    # like one continuous piece of channel art.
    room = load_avatar("liveavatar-060843-clean.png")
    cover_scale = max(2560 / room.width, 1440 / room.height)
    room_cover = room.resize((int(room.width * cover_scale), int(room.height * cover_scale)), Image.Resampling.LANCZOS)
    cover_x = (room_cover.width - 2560) // 2
    cover_y = (room_cover.height - 1440) // 2
    room_cover = room_cover.crop((cover_x, cover_y, cover_x + 2560, cover_y + 1440))
    room_cover = ImageEnhance.Brightness(room_cover).enhance(0.62)
    room_cover = ImageEnhance.Color(room_cover).enhance(0.66)
    room_cover = room_cover.filter(ImageFilter.GaussianBlur(16))
    img.alpha_composite(room_cover)

    shade = Image.new("RGBA", img.size, (0, 0, 0, 0))
    sd = ImageDraw.Draw(shade)
    sd.rectangle((0, 0, 1370, 1440), fill=(16, 6, 2, 128))
    sd.rectangle((1180, 0, 2560, 1440), fill=(0, 0, 0, 56))
    shade = shade.filter(ImageFilter.GaussianBlur(90))
    img.alpha_composite(shade)

    draw_logo_lockup(img, (1002, 720), 540, leap_size=56, gap=20)
    avatar = face_crop("liveavatar-060843-clean.png", (34, 0, 625, 990))
    ratio = 1110 / avatar.height
    resized = avatar.resize((int(avatar.width * ratio), 1110), Image.Resampling.LANCZOS)
    mask = soft_mask(resized.size, edge=170)
    avatar_x = 1688 - resized.width // 2
    avatar_y = 955 - resized.height // 2
    img.paste(resized, (avatar_x, avatar_y), mask)
...
def make_proof(paths: Sequence[Path]) -> Path:
    ...
    d.text((gap, 34), "aiASAP social artwork v12 draft", font=font(FONT_BOLD, 52), fill=COPY)
    d.text(
        (gap, 98),
    ...
    return save(sheet, "aiasap-social-artwork-v12-proof-sheet.png")
...
def make_manifest() -> None:
    ...
    assets.insert(
        0,
        {
            ...
            "size": "review",
            "use": "One-sheet desktop review of the v12 draft",
            "path": str(OUT / "aiasap-social-artwork-v12-proof-sheet.png"),
        },
    )
    payload = {
        "version": "v12",
        ...
        "approved_rules": [
            ...
            "Logo style source: app/globals.css .aiasap-logo-mark, Arial Black stack, italic, tracking 0, stronger synthetic italic lean to match the mobile browser look, scaleX(0.94), brown-gold gradient.",
...
def main() -> None:
    ...
    make_manifest()
    print(f"Wrote v12 artwork to {OUT}")
