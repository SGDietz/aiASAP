...
export function speakThroughAvatar(
  ...
  const rescue = (reason: string) => {
    ...
    onRescue?.(reason);
    // Do not push repeatAudio or WebAudio into the avatar session here. That
    // rescue path produced rapid start/end events, no lip sync, and repeated
    // mic CLOSED/OPEN clicks. The stage records the fault but never changes
    // modes without the visitor pressing the explicit Voice control.
    reportAvatarSpeechFailure({ session, where, reason });
  };
  ...
  const handleStarted = (event?: { event_id?: string }) => {
  ...
  try {
    ...
    expectedEventId = typeof result === "string" ? result : null;
