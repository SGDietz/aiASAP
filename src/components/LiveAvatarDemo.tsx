...
export const LiveAvatarDemo = () => {
  ...
  // A blocked site is recovered in browser settings, never by re-asking. These
  // two only drive the recovery card's own button while that re-read is in
  // flight; neither can start a session.
  ...
  // Last observed browser-level block. A ref, not state, so the check below
  // cannot read a stale closure.
  //
  // Measured 2026-08-28: while the site was blocked, three taps on START fired
  // three mints. The permission can never be granted in that state, so every
  // one of those was a paid session created and thrown away. The recovery
  // card's own button was already free; START was not.
  ...
  const beginSessionFromStart = useCallback(async (fromStopped = false) => {
    ...
    // Known-blocked re-tap. Handle it BEFORE clearing any card state: clearing
    // first unmounted the card for the whole length of the re-check, so the
    // screen went blank and then the card reappeared. That blink is what reads
    // as "I hit START and it does nothing" (Chief relaying G, 2026-08-28).
    //
    // Permissions.query is free and never prompts. If the block is still there,
    // say so and spend nothing — a mint could not be used. Measured before this
    // guard existed: three taps while blocked minted three paid sessions.
    if (micBlockedRef.current) {
      // Hold the acknowledgement briefly. The read returns in single-digit
      // milliseconds, so without a floor the button never visibly changes and
      // the tap still reads as ignored.
    ...
    beginStartupTiming();
    // Clear all prior attempt state before this fresh, gesture-owned request.
    // Neither a historical browser query nor a previous stopped session may
    // put blocked text on a clean HTTPS START screen.
    setStartMicGranted(false);
  ...
  /**
   * The recovery button for a BLOCKED site.
   *
   * It must never call getUserMedia and must never mint. A browser does not
   * prompt twice for a blocked site, so a re-request is guaranteed to fail,
   * and the old handler ran the full START path — which fired
   * /api/start-custom-session on every single tap and billed for a session it
   * then aborted. G hit that on Android on 2026-08-28; reproduced headless with
   * the mint held.
   *
   * Permissions.query is a free read that never prompts. Only once the visitor
   * has actually flipped the setting does this hand back to the real,
   * gesture-owned START path.
   */
  const recheckBlockedMicrophone = useCallback(async () => {
    ...
    try {
      ...
      if (observed === "denied" || observed === "unavailable") {
        // Nothing changed on the browser side. Say so and spend nothing.
        setStartMicPermissionState(observed === "denied" ? "denied" : "unavailable");
        setStartMicStillBlocked(observed === "denied");
        return;
      }
      // The block is gone. Clear the card, then run the ordinary START path so
      // the request stays owned by this tap.
      setStartMicPermissionState(null);
      setStartMicStillBlocked(false);
