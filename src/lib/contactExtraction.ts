...
function hasSpokenEmailCue(text: string): boolean {
  const t = text.toLowerCase();
  if (/\b(?:email|e-mail|mail address|email address|reach me at|send it to)\b/.test(t)) {
    return true;
  }
  return /\bat\s+(?:gmail|yahoo|hotmail|outlook|icloud|protonmail|pm|aol)\s+dot\b/.test(t);
}
...
export function isGarbageNameCandidate(s: string | null | undefined): boolean {
  ...
  if (/\b(talking to codex|look at|number \d+|on the list)\b/i.test(lower)) {
    return true;
  }
...
export function extractContactDetails(text: string): ContactExtraction {
  ...
  const emailMatch =
    ...
    (hasSpokenEmailCue(text) ? spokenEmailText.match(EMAIL_REGEX) : null);
...
export function detectFollowUpIntent(text: string): {
  ...
  return {
    interested: interestedKeywords.some(hasPhrase),
    declined: declinedKeywords.some(hasPhrase),
  };
}
