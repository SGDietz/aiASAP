...
describe("phone start-screen avatar lock", () => {
  ...
  it("leaves the still source, branding, and stage anchors untouched", () => {
    ...
    expect(css).toMatch(/\[data-six-initial-idle="1"\] \.aiasap-brand-lockup,[\s\S]*?\[data-six-active-stage="1"\] \.aiasap-brand-lockup \{\s*transform: translateY\(-8px\);/);
