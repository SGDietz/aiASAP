...
describe("shared visible loading treatment", () => {
  it("uses the canonical splash mark with one visible universal Loading label", () => {
    ...
    const loadingMotion = css.slice(
      css.indexOf("/* The word and complete badge"),
      css.indexOf('[data-six-loading-mark="1"]'),
    );
    const loadingInkGeometryStart = css.indexOf('[data-six-loading-ink="1"] {');
    const loadingInkGeometry = css.slice(
      loadingInkGeometryStart,
      css.indexOf("}", loadingInkGeometryStart) + 1,
    );
    ...
    expect(loadingMotion).toContain('six-loading-light-chase 2.6s ease-in-out infinite alternate');
    expect(loadingMotion).toContain('six-loading-focus-pulse 1.6s ease-in-out infinite');
    ...
    expect(loadingInkGeometry).not.toContain("background-image:");
    expect(css).toMatch(/@media \(prefers-reduced-motion: reduce\) \{[\s\S]*?\[data-six-loading-ink="1"\],[\s\S]*?\[data-six-loading-shimmer-band="1"\] \{[\s\S]*?animation: none;[\s\S]*?will-change: auto;/);
    expect(css).toMatch(/@media \(prefers-reduced-motion: reduce\)[\s\S]*?\[data-six-loading-ink="1"\] \{[\s\S]*?background-image: linear-gradient\(to bottom, #ffe9c2, #d7a05a, #8c5f30\);/);
    expect(css).toMatch(/@media \(prefers-reduced-motion: reduce\)[\s\S]*?\[data-six-loading-shimmer-band="1"\] \{[\s\S]*?opacity: 0;/);
    expect(css).not.toMatch(/@keyframes six-loading-(?:light-chase|focus-pulse)[\s\S]{0,600}?transform:/);
    expect(loader).toContain('data-six-loading-shimmer-band="1"');
    expect(loader).toContain('id="six-loading-badge-shimmer"');
    expect(css).toContain('six-loading-badge-shimmer 2.6s cubic-bezier(0.45, 0, 0.55, 1) infinite');
    expect(css).toContain('six-loading-badge-breathe 1.6s ease-in-out infinite');
    expect(css).toContain('six-loading-rim-shimmer 1.6s ease-in-out infinite');
  ...
  it("keeps the branded stage header, warm surround, and legal footer around site and Six startup", () => {
    ...
    expect(source("src/components/StageBrandLockup.tsx")).toContain("text-[calc(var(--stage-width)*0.10)]");
