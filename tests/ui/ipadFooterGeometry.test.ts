...
describe("iPad legal separation across lifecycle states", () => {
  it("keeps the accepted legal line inside iPad portrait", () => {
    const rects = ipadRects(1366);
    expect(rects.legalTop - rects.rimBottom).toBeCloseTo(6, 5);
    expect(rects.legalBottomUpperBound).toBeLessThanOrEqual(1366);
  });

  it("moves only the iPad portrait composition north and parks contacts inside its rim", () => {
    const css = source("app/globals.css");
    const portrait = css.slice(css.indexOf("/* iPad portrait refinement"));
    expect(portrait).toContain("/ 2 - 10px");
    expect(portrait).toContain("transform: translateY(-10px)");
    expect(portrait).toContain("/ 2 + 18px");
    expect(portrait).toContain("var(--stage-height) + 14px");
    expect(portrait).not.toContain("(orientation: landscape)");
  });
