...
describe("START/RUNNING literal visual reuse and universal Loading", () => {
  ...
  it("reuses START tagline and footer placement in RUNNING", () => {
    const session = source("src/components/LiveAvatarSession.tsx");
    ...
    expect(lockup).toContain('className="aiasap-tablet-idle-tagline -mt-1');
    expect(lockup).toContain("font-[540]");
  ...
  it("has no responsive utility competitor on the shared START/RUNNING branch", () => {
    ...
    expect(controls).toContain('mobileStartControls ? "h-[150px] w-[220px] grid-rows-2 gap-0"');
  });

  it("keeps one label owner and exact top/bottom row latitude pairing", () => {
    const controls = source("src/components/StageControls.tsx");
    const css = source("app/globals.css");
    ...
    expect(btnOwner.match(/data-stage-control-label="1"/g)).toHaveLength(2);
    expect(btnOwner).toContain("className={MOBILE_START_LABEL}");
