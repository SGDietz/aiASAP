...
describe("single-6 icon and consolidated legal contracts", () => {
  ...
  it("renders one compact Your Rights control beside copyright and Terms without changing footer geometry", () => {
    ...
    expect(footer).toContain("Your Rights");
    expect(footer).toContain('href="/legal"');
    expect(footer).toContain("Terms/Legal");
    expect(footer).not.toContain("Terms / Legal");
    expect(footer).toContain("inline-block origin-center scale-[1.6]");
    expect(footer).toContain("text-[#d7a05a]");
    expect(footer.match(/<span aria-hidden/g)).toHaveLength(2);
    expect(footer).toContain("text-[clamp(10px,2.8vw,14px)]");
    ...
    expect(footer).toContain('aria-label="Open Your Rights"');
  ...
  it("makes Your Rights proud, paid-project specific, and legally bounded", () => {
    ...
    expect(rights).toContain("When a project or project phase is completed and paid for, you own the client-specific work we delivered for that phase—and everything you build from it.");
    expect(rights).toContain("aiASAP keeps no ownership, royalties, equity, profit share, or claim on your future success.");
    expect(rights).toContain("Work from unpaid future phases is not included.");
    expect(rights).toContain("Pre-existing aiASAP tools and third-party materials remain subject to the licenses explained in the agreement.");
