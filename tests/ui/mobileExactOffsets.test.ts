...
describe("exact phone-only stage offsets", () => {
  ...
  it("keeps the shared phone lift and raises only initial-idle branding exactly 4px", () => {
    ...
    const phoneBlock = css.slice(
      ...
      css.indexOf("@keyframes fade-opacity"),
    );

    expect(phoneBlock).toMatch(
      /\.aiasap-brand-lockup\s*\{\s*transform: translateY\(-2px\);\s*\}/,
    );
    expect(demo.match(/aiasap-brand-lockup/g)).toHaveLength(6);
    expect(session.match(/aiasap-brand-lockup/g)).toHaveLength(1);
    expect(css).toMatch(
      /\[data-six-initial-idle="1"\] \.aiasap-brand-lockup\s*\{\s*transform: translateY\(-8px\);\s*\}/,
    );
    expect(css.match(/\.aiasap-brand-lockup/g)).toHaveLength(2);
  });
  ...
  it("lifts only the complete phone legal line by exactly 4px", () => {
    ...
    expect(footer.match(/opacity-\[0\.9\]/g)).toHaveLength(1);
  ...
  it("keeps the initial-idle controls authority and centers the legal line", () => {
    ...
    expect(css).toMatch(
      /\[data-six-initial-idle="1"\] \[data-stage-controls="1"\]\.stage-controls-cluster\s*\{\s*transform: translateX\(-50%\);\s*\}/,
    );
    expect(css).toMatch(
      /\[data-six-initial-idle="1"\] \[data-stage-legal-line="1"\]\s*\{\s*transform: none;\s*opacity: 0\.7;\s*\}/,
    );
  });
});
